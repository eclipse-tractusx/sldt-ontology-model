...
public class OntologyConverter implements Converter {
    ...
    public void convert() {
        ...
        logger.info("Converting Ontology:");
        if (!initialized) {
            logger.info("  -> preLoadOntontology()"+new java.text.SimpleDateFormat("HH:mm:ss").format(java.util.Calendar.getInstance().getTime()));
            preLoadOntology();
        }
        ...
        try {
            ...
            logger.info("  -> postParsing() "+new java.text.SimpleDateFormat("HH:mm:ss").format(java.util.Calendar.getInstance().getTime()));
            postParsing(ontology, vowlData, manager);

            this.addLoadingInfoToParentLine("... done");
            logger.info("--- Finished --- "+new java.text.SimpleDateFormat("HH:mm:ss").format(java.util.Calendar.getInstance().getTime()));
    ...
    /**
     * Ex the generated data according to the implemented {@link Exporter}.
     *
     * @param exporter
     *            The exporter.
     * @throws Exception
